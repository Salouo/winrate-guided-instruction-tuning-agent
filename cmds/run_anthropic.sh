...
echo "[SCRIPT] run_anthropic_duel.sh :: train->eval (Executor=Anthropic/Haiku; Scorer&Optimizer=OpenAI)"
...
# ===== Train settings =====
ANTHROPIC_CONCURRENCY="${ANTHROPIC_CONCURRENCY:-8}"   # default 8
EXEC_TEMPERATURE="${EXEC_TEMPERATURE:-0}"
SCOR_TEMPERATURE="${SCOR_TEMPERATURE:-0}"
EXTRA_TRAIN_FLAGS="${EXTRA_TRAIN_FLAGS:-}"

# ===== Eval settings =====
ANTHROPIC_EVAL_CONCURRENCY="${ANTHROPIC_EVAL_CONCURRENCY:-$ANTHROPIC_CONCURRENCY}"
...
for dataset in "${DATASETS[@]}"; do
  ...
  train_desc="TRAIN [executor=anthropic | scorer=openai | optimizer=openai | $eval_dir | $dataset]"
  ...
  if [[ -n "$LOG_DIR" ]]; then
    train_cmd=(bash -c "uv run python -u -m src.train \
      ...
      ${EXTRA_TRAIN_FLAGS} 2>&1 | tee \"${LOG_DIR}/anthropic-openai-openai__${eval_dir}__${dataset}__train.log\"")
  else
  ...
  if run_with_retries "$RETRIES" "$RETRY_SLEEP" "$train_desc" "${train_cmd[@]}"; then
    ...
    TRAIN_FAILS+=("anthropic-openai-openai:$eval_dir:$dataset")
  fi
done
