...
# ===== Eval settings =====
OPENAI_EVAL_CONCURRENCY="${OPENAI_EVAL_CONCURRENCY:-8}"
EXEC_TEMPERATURE="${EXEC_TEMPERATURE:-0}"
SCOR_TEMPERATURE="${SCOR_TEMPERATURE:-0}"
EVAL_Z="${EVAL_Z:-1.96}"
EXTRA_EVAL_FLAGS="${EXTRA_EVAL_FLAGS:-}"

# 固定 max_hist（可用环境变量覆盖）
MAX_HIST="${MAX_HIST:-0}"
EXTRA_EVAL_FLAGS="--max-hist $MAX_HIST ${EXTRA_EVAL_FLAGS}"
...
if (( $# >= 1 )); then
  # Remaining args are explicit dataset names
  SELECTED=("$@")
else
  # Optional filter by DS_GLOB (comma/space separated patterns)
  if [[ -n "${DS_GLOB:-}" ]]; then
  ...
  # If still empty, interactive selection
  if (( ${#SELECTED[@]} == 0 )); then
...
# Concurrency flag (only inject if EXTRA_EVAL_FLAGS does NOT contain --concurrency)
extra_eval_conc=""
[[ -n "${OPENAI_EVAL_CONCURRENCY:-}" && "$EXTRA_EVAL_FLAGS" != *"--concurrency"* ]] && extra_eval_conc="--concurrency $OPENAI_EVAL_CONCURRENCY"

# Force accepted-only unless user explicitly provided it
...
for dataset in "${SELECTED[@]}"; do
  ...
  eval_desc="EVAL-INSTS [openai | $eval_dir | $dataset]"
  if [[ -n "$LOG_DIR" ]]; then
    eval_cmd=(bash -c "uv run python -u -m src.eval \
      ...
      ${EXTRA_EVAL_FLAGS} 2>&1 | tee \"${LOG_DIR}/openai__${eval_dir}__${dataset}__eval_insts.log\"")
  else
  ...
  if ! run_with_retries "$RETRIES" "$RETRY_SLEEP" "$eval_desc" "${eval_cmd[@]}"; then
    EVAL_FAILS+=("openai:$eval_dir:$dataset:eval_insts")
  fi
done
